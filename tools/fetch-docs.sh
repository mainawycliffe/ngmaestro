...
# Directory setup
DOCS_DIR="$(pwd)/docs"
TEMP_DIR="$(pwd)/tmp/angular-repo"

# URLs and Paths
REPO_URL="https://github.com/angular/angular.git"
CONTENT_PATH="adev/src/content"

# Versions mapping (Version:Branch)
...
echo "Cloning Angular repository (sparse)..."
...
for ENTRY in "${VERSIONS[@]}"; do
  VERSION="${ENTRY%%:*}"
  BRANCH="${ENTRY##*:}"
  TARGET_DIR="$DOCS_DIR/$VERSION"
  
  echo "------------------------------------------------"
  echo "Fetching docs for $VERSION (Branch: $BRANCH)..."
  echo "------------------------------------------------"
